...
<div class="flex h-full w-full flex-1 flex-col gap-6 p-4 sm:p-6 md:p-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">All Transactions</h1>
            <p class="text-sm text-slate-600 dark:text-slate-400">View, audit, export, and manage all sales records across registers</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div class="relative rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-900/70 p-5 sm:p-6 backdrop-blur-xl shadow-lg dark:shadow-xl hover:border-orange-500/40 transition-all duration-300 group overflow-hidden">
            <div class="flex items-center justify-between">
                <div class="space-y-1">
                    ...
                    <p class="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ number_format($this->totalQuantity, 2) }} <span class="text-sm font-normal text-slate-500 dark:text-slate-400">kg</span></p>
                </div>
        ...
        <div class="relative rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-900/70 p-5 sm:p-6 backdrop-blur-xl shadow-lg dark:shadow-xl hover:border-orange-500/40 transition-all duration-300 group overflow-hidden">
            <div class="flex items-center justify-between">
                <div class="space-y-1">
                    <p class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Active Cashiers</p>
                    <p class="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ $this->cashiers->count() }}</p>
                </div>
    ...
    <div class="relative rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-900/70 backdrop-blur-xl shadow-xl dark:shadow-2xl overflow-hidden">
        <div class="border-b border-slate-200/80 dark:border-slate-800/80 px-6 py-5 bg-slate-50/60 dark:bg-slate-950/40">
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">System audit log of all completed, cancelled, and refunded sales</p>
        </div>
        ...
        <div class="p-0">
                            ...
                            @foreach($this->transactions as $transaction)
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <div class="font-medium text-slate-900 dark:text-white">{{ $transaction->cashier->name }}</div>
                                        <div class="text-xs text-slate-500 dark:text-slate-400">{{ $transaction->cashier->email }}</div>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        ...
                                        <div class="text-xs text-slate-500 dark:text-slate-400">{{ $transaction->formatted_price_per_kg }}</div>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4 text-xs">
                                        <div class="font-medium text-slate-900 dark:text-white">{{ $transaction->customerDiscount->name ?? 'None' }}</div>
                                        @if($transaction->customerDiscount && $transaction->customerDiscount->discount_per_kg > 0)
                                            <div class="text-red-600 dark:text-red-400 font-semibold">-{{ $transaction->customerDiscount->formatted_discount }}</div>
                                        @endif
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
            ...
            @else
                <div class="text-center py-12 px-4">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Try adjusting search query or cashier filters.</p>
                </div>
            @endif
        </div>
    </div>
    ...
    @if($showTransactionModal && $selectedTransaction)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-md p-4 animate-in fade-in duration-200">
            <div class="relative w-full max-w-2xl rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white/95 dark:bg-slate-900/95 p-6 backdrop-blur-xl shadow-2xl overflow-hidden">
                <!-- Top Accent Line -->
                <div class="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-orange-500 via-amber-500 to-orange-600"></div>
                ...
                <div class="mb-5 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs font-mono text-orange-600 dark:text-orange-400 mt-0.5">{{ $selectedTransaction->transaction_number }}</p>
                    </div>
                    <button wire:click="closeTransactionModal" class="rounded-xl border border-slate-300 dark:border-slate-700 px-3.5 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer">
                        Close
                    </button>
                </div>
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div class="space-y-4">
                        <div>
                            <h4 class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Sale Details</h4>
                            <div class="space-y-2 text-xs bg-slate-100/80 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-700/50">
                                <div class="flex justify-between">
                                    <span class="text-slate-500 dark:text-slate-400">Status:</span>
                                    <span class="font-bold {{ $selectedTransaction->status === 'cancelled' ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400' }}">{{ ucfirst($selectedTransaction->status) }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-slate-500 dark:text-slate-400">Date:</span>
                                    <span class="font-medium text-slate-900 dark:text-white">{{ $selectedTransaction->created_at->format('M d, Y H:i') }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-slate-500 dark:text-slate-400">Quantity:</span>
                                    <span class="font-semibold text-slate-900 dark:text-white">{{ $selectedTransaction->formatted_quantity }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-slate-500 dark:text-slate-400">Original Price:</span>
                                    <span class="font-medium text-slate-800 dark:text-slate-200">{{ $selectedTransaction->formatted_price_per_kg }}</span>
                                </div>
                                @if($selectedTransaction->customerDiscount && $selectedTransaction->customerDiscount->discount_per_kg > 0)
                                    <div class="flex justify-between">
                                        <span class="text-slate-500 dark:text-slate-400">Discount ({{ $selectedTransaction->customerDiscount->name }}):</span>
                                        <span class="font-semibold text-red-600 dark:text-red-400">-{{ $selectedTransaction->customerDiscount->formatted_discount }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-slate-500 dark:text-slate-400">Effective Price:</span>
                                        <span class="font-semibold text-slate-900 dark:text-white">{{ $selectedTransaction->formatted_effective_price_per_kg }}</span>
                                    </div>
                                @endif
                                <div class="flex justify-between pt-2 border-t border-slate-200 dark:border-slate-700">
                                    ...
                                    <span class="text-sm font-extrabold text-emerald-600 dark:text-emerald-400">{{ $selectedTransaction->formatted_total }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        @if($selectedTransaction->status === 'cancelled')
                            <div class="rounded-2xl border border-red-500/30 bg-red-500/10 p-4 text-xs">
                                ...
                                <div class="mt-2 space-y-1 text-red-700 dark:text-red-300">
                                    <p class="font-medium">{{ $selectedTransaction->cancellation_reason ?: 'No reason provided' }}</p>
                                    @if($selectedTransaction->cancelled_at)
                                        <p class="text-[11px] text-red-500 dark:text-red-400">Cancelled: {{ $selectedTransaction->cancelled_at->format('M d, Y H:i:s') }}</p>
                                    @endif
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="space-y-4">
                        <div>
                            <h4 class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Customer & Cashier</h4>
                            <div class="space-y-3 text-xs bg-slate-100/80 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-700/50">
                                <div>
                                    <span class="text-slate-500 dark:text-slate-400 block mb-0.5">Customer Name:</span>
                                    <span class="font-bold text-slate-900 dark:text-white">{{ $selectedTransaction->customer_name ?: 'Walk-in Customer' }}</span>
                                    @if($selectedTransaction->customer_phone)
                                        <span class="text-slate-500 dark:text-slate-400 block">{{ $selectedTransaction->customer_phone }}</span>
                                    @endif
                                </div>
                                <div class="pt-2 border-t border-slate-200 dark:border-slate-700">
                                    <span class="text-slate-500 dark:text-slate-400 block mb-0.5">Cashier:</span>
                                    <span class="font-bold text-slate-900 dark:text-white">{{ $selectedTransaction->cashier->name }}</span>
                                    <span class="text-slate-500 dark:text-slate-400 block">{{ $selectedTransaction->cashier->email }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        @if($selectedTransaction->notes)
                            <div>
                                <h4 class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1">Notes</h4>
                                <div class="text-xs text-slate-700 dark:text-slate-300 bg-slate-100/80 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-200 dark:border-slate-700/50">
