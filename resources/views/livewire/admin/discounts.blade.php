...
<div class="flex h-full w-full flex-1 flex-col gap-6 p-4 sm:p-6 md:p-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Customer Discounts</h1>
            <p class="text-sm text-slate-600 dark:text-slate-400">Configure customer tiers, per-kg price reductions, and default discount rules</p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 items-end">
            ...
            <div>
                <div class="rounded-xl bg-slate-100/80 dark:bg-slate-800/60 p-2.5 border border-slate-200/80 dark:border-slate-700/60 text-xs font-semibold text-slate-700 dark:text-slate-300 text-center">
                    Total Active Discount Rules: <strong class="text-orange-600 dark:text-orange-400">{{ $this->discounts->total() }}</strong>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="relative rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-900/70 backdrop-blur-xl shadow-xl dark:shadow-2xl overflow-hidden">
        <div class="border-b border-slate-200/80 dark:border-slate-800/80 px-6 py-5 bg-slate-50/60 dark:bg-slate-950/40">
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">Discount per kg applied automatically during register checkout</p>
        </div>
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                                    <td class="whitespace-nowrap px-6 py-4">
                                        ...
                                        @if($discount->description)
                                            <div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ Str::limit($discount->description, 50) }}</div>
                                        @endif
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4 font-mono">
                                        <span class="font-bold text-emerald-600 dark:text-emerald-400">-{{ $discount->formatted_discount }}</span>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        @if($discount->is_default)
                                            <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20">
                                                Default Rule
                                            </span>
                                        @else
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <div class="font-semibold text-slate-900 dark:text-white">{{ $discount->transactions()->count() }} sales</div>
                                    </td>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-md p-4 animate-in fade-in duration-200">
            <div class="relative w-full max-w-md rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white/95 dark:bg-slate-900/95 p-6 backdrop-blur-xl shadow-2xl overflow-hidden">
                ...
                <div class="mb-5">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">Configure discount per kg rate and default rules</p>
                </div>
