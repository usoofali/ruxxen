...
<div class="flex h-full w-full flex-1 flex-col gap-6 p-4 sm:p-6 md:p-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-xl sm:text-xl font-extrabold tracking-tight text-slate-900 dark:text-white">Sales History
            </h1>
            <p class="text-sm text-slate-600 dark:text-slate-400">View, search, and reprint past transaction receipts
            </p>
        </div>
    </div>
    ...
    <div
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="flex items-end">
                <flux:button wire:click="$set('search', ''); $set('dateFilter', ''); $set('statusFilter', '')"
                    variant="outline"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-xl sm:text-xl font-extrabold text-slate-900 dark:text-white tracking-tight">
                        {{ number_format($this->totalQuantity, 2) }} <span
                            class="text-base font-normal text-slate-500 dark:text-slate-400">kg</span></p>
                </div>
                <div
        ...
        <!-- Transactions Count -->
        <div
            class="relative rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-900/70 p-5 sm:p-6 backdrop-blur-xl shadow-lg dark:shadow-xl hover:border-orange-500/40 transition-all duration-300 group overflow-hidden sm:col-span-2 lg:col-span-1">
            <div class="flex items-center justify-between">
                <div class="space-y-1">
                    <p class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
        ...
        <div class="p-0">
                                ...
                                ($transaction->status === 'cancelled' ? 'bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/20' :
                                    'bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20') }}">
                                                ...
                                                <td class="whitespace-nowrap px-6 py-4 text-right">
                                                    <button wire:click="openReceipt({{ $transaction->id }})"
            ...
            @else
                <div class="text-center py-12 px-4">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Try adjusting your search query or date
                        filters.</p>
                </div>
            @endif
        </div>
            ...
            <div
                ...
                <div
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-3">
                                @if(\App\Services\CompanySettingsService::getCompanyLogoUrl())
                                    <img src="{{ \App\Services\CompanySettingsService::getCompanyLogoUrl() }}" alt="Company Logo"
                                ...
                                @else
                                    <div
                                        class="h-12 w-12 rounded-xl bg-gradient-to-br from-orange-500 to-amber-600 flex items-center justify-center text-white shadow-md">
                                        <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="text-right">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono font-semibold bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20">
                    ...
                    <div class="py-4 space-y-4">
                        <div>
                            <div
                                class="bg-slate-100/80 dark:bg-slate-800/50 rounded-2xl p-3.5 border border-slate-200/80 dark:border-slate-700/50 text-xs">
                                @if($currentTransaction->customer_name)
                                    <div class="flex justify-between items-center">
                                        <span class="text-slate-500 dark:text-slate-400">Customer:</span>
                                        <span
                                            class="font-semibold text-slate-900 dark:text-white">{{ $currentTransaction->customer_name }}</span>
                                    </div>
                                    @if($currentTransaction->customer_phone)
                                        <div class="flex justify-between items-center mt-1">
                                            <span class="text-slate-500 dark:text-slate-400">Phone:</span>
                                            <span
                                                class="font-medium text-slate-700 dark:text-slate-300">{{ $currentTransaction->customer_phone }}</span>
                                        </div>
                                    @endif
                                @else
                        ...
                        <div class="space-y-2 text-xs">
                            <div class="flex justify-between py-1 border-b border-slate-200/60 dark:border-slate-800/60">
                                ...
                                <span
                                    class="font-semibold text-slate-900 dark:text-white">{{ $currentTransaction->formatted_quantity }}</span>
                            </div>
                            <div class="flex justify-between py-1 border-b border-slate-200/60 dark:border-slate-800/60">
                                <span class="text-slate-500 dark:text-slate-400">Unit Price:</span>
                                <span
                                    class="font-medium text-slate-800 dark:text-slate-200">{{ $currentTransaction->formatted_price_per_kg }}</span>
                            </div>
                            <div class="flex justify-between py-1 border-b border-slate-200/60 dark:border-slate-800/60">
                                <span class="text-slate-500 dark:text-slate-400">Payment Method:</span>
                                <span
                                    class="font-medium text-slate-800 dark:text-slate-200">{{ ucfirst($currentTransaction->payment_type) }}</span>
                            </div>
                            ...
                            @if($currentTransaction->status === 'cancelled')
                                <div class="mt-3 rounded-2xl border border-red-500/30 bg-red-500/10 p-3 text-xs">
                                    ...
                                    <div class="mt-1 text-red-700 dark:text-red-300 font-medium">
                                        {{ $currentTransaction->cancellation_reason ?: 'No reason provided' }}</div>
                                </div>
                            @endif
                            ...
                            <div class="flex justify-between pt-2">
                                ...
                                <span
                                    class="text-base font-extrabold text-emerald-600 dark:text-emerald-400">{{ $currentTransaction->formatted_total }}</span>
                            </div>
                        </div>

                        <div
                            class="text-center text-[11px] text-slate-500 dark:text-slate-400 pt-2 border-t border-slate-200/80 dark:border-slate-800/80">
