            ...
            <div class="flex w-full max-w-md flex-col gap-6">
                <a href="{{ route('home') }}" class="flex flex-col items-center gap-2 font-medium" wire:navigate>
                    @if(\App\Services\CompanySettingsService::getCompanyLogoUrl())
                        <span class="flex h-12 w-12 items-center justify-center rounded-md overflow-hidden">
                        ...
                        @if(\App\Services\CompanySettingsService::getCompanyName())
                            <span class="text-sm font-medium text-gray-900 dark:text-white">{{ \App\Services\CompanySettingsService::getCompanyName() }}</span>
                        @endif
                    @else
